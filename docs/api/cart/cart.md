...
- **One-cart-per-user concurrency** — The service enforces one cart per user at the application level. Concurrent first-adds are serialized inside the transaction (e.g., a PostgreSQL advisory lock keyed by the user's internal ID, or a check-then-create pattern); a future unique index on `carts.users_id` is documented as the database-level hardening.
