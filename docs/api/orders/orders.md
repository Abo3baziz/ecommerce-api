...
- **Coupon support at checkout** — The `coupons`/`coupon_usages` tables exist and the requirements mention discounts, so checkout accepts an optional `coupon_code` and validates it transactionally (active, in window, usage limits, minimum order) against the schema columns. `coupon_code` is matched case-insensitively and normalized to uppercase on input. Coupon **management** (admin CRUD) is out of scope; rows are seeded out-of-band.
